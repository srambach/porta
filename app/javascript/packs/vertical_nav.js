...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const currentApi = dataset.current_api && safeFromJsonString(dataset.current_api)

  // TODO: where does this go?
  // - if can?(:manage, :plans)
  //   { title: 'Integration Errors', path: admin_service_errors_path(@service) },
  VerticalNav({ sections, activeSection, activeItem, currentApi }, 'vertical-nav-wrapper')
})
